...
class UserApprovalInterface:
    ...
    def get_change_approval(self, change: ConfigChange) -> bool:
        ...
        while True:
            response = input(
                "\nApprove this change? [y]es/[n]o/[s]how full content/[q]uit: "
            ).lower().strip()
    ...
    def get_batch_approval(self, changes: List[ConfigChange]) -> Dict[Path, bool]:
        ...
        print(f"\n{'='*60}")
        print(f"Configuration Changes Summary")
        print(f"{'='*60}")
        ...
        while True:
            response = input(
                "Approve all changes? [y]es/[n]o/[r]eview individually/[q]uit: "
            ).lower().strip()
    ...
    def resolve_conflicts_interactively(self, conflicts: List[Conflict]) -> List[Conflict]:
        ...
        for i, conflict in enumerate(conflicts, 1):
            ...
            while True:
                choice = input(
                    "\nResolve conflict: [k]eep existing/[u]se template/[m]anual/[s]kip: "
                ).lower().strip()
    ...
    def show_dry_run_results(self, changes: List[ConfigChange]) -> None:
        ...
        for change in changes:
            ...
            # Show brief diff
            if change.old_content is not None:
                stats = self.diff_generator.get_diff_stats(
