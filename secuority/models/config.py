...
class ConfigChange:
    ...
    def validate(self) -> bool:
        ...
        try:
            ...
            if (self.change_type == ChangeType.UPDATE and
                    not validate_file_path(self.file_path)):
                return False

            # Validate that old_content exists for UPDATE operations
            if (self.change_type == ChangeType.UPDATE and
                    self.old_content is None):
                return False
            ...
            'failed_changes': [
                {
                    ...
                    'error': str(error)
                } for change, error in self.failed_changes
            ],
            'conflicts': [conflict.to_dict() for conflict in self.conflicts],
...
class ChangeSet:
    ...
    def get_change_by_path(self, file_path: Path) -> Optional[ConfigChange]:
