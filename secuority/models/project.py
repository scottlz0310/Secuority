...
class ProjectState:
    ...
    def validate(self) -> bool:
        ...
        try:
            ...
            if (self.dependency_analysis and
                    not isinstance(self.dependency_analysis, DependencyAnalysis)):
                return False
    ...
    def needs_dependency_migration(self) -> bool:
        ...
        return (self.has_requirements_txt and
                self.has_pyproject_toml and
                self.dependency_analysis is not None and
                self.dependency_analysis.migration_needed)
    
    def get_missing_security_tools(self) -> List[SecurityTool]:
        """Get list of missing security tools."""
        return [tool for tool, enabled in self.security_tools.items()
                if not enabled]

    def get_missing_quality_tools(self) -> List[QualityTool]:
        """Get list of missing quality tools."""
        return [tool for tool, enabled in self.quality_tools.items()
                if not enabled]
    ...
    def get_dependency_manager_from_files(self) -> Optional[DependencyManager]:
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            'dependency_manager': (self.dependency_manager.value
                                   if self.dependency_manager else None),
            'current_tools': {
                name: {
                    'name': config.name,
            ...
            'security_tools': {
                tool.value: enabled
                for tool, enabled in self.security_tools.items()
            },
            'quality_tools': {
                tool.value: enabled
                for tool, enabled in self.quality_tools.items()
            },
            'ci_workflows': [
                {
                    'name': w.name,
    ...
    def from_dict(cls, data: Dict) -> 'ProjectState':
        ...
        basic_attrs = [
            'has_pyproject_toml', 'has_requirements_txt', 'has_setup_py',
            'has_gitignore', 'has_pre_commit_config', 'python_version'
        ]
