    ...
    def check_security_settings(self, owner: str, repo: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'secret_scanning_push_protection': security_analysis.get('secret_scanning_push_protection', {}).get('status') == 'enabled',
                'dependency_graph': repo_data.get('has_vulnerability_alerts', False),
                'private_vulnerability_reporting': security_analysis.get('private_vulnerability_reporting', {}).get('status') == 'enabled'
            }
        except GitHubAPIError:
            raise
