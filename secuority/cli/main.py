...
def check(
    ...
    try:
        ...
        if (project_state.dependency_analysis and 
        ...
        logger.debug("Project statistics", python_files=python_files_count, total_files=len(list(project_path.glob('**/*'))))
        ...
        if not structured_output:
            ...
            if verbose:
                ...
                if github_analysis and github_analysis.get("analysis_successful"):
                    console.print(f"  • GitHub repository: {github_analysis.get('owner')}/{github_analysis.get('repo')}")
...
def apply(
    ...
    security_only: bool = typer.Option(
        False,
        "--security-only",
        help="Apply only security-related configurations"
    ),
    templates_only: bool = typer.Option(
        False,
        "--templates-only",
        help="Apply only template-based configurations"
    ),
) -> None:
    ...
    try:
        logger.info("Starting configuration application", 
        ...
        if conflicted_changes and not dry_run:
            if not structured_output:
                console.print(f"[yellow]⚠ Found {len(conflicted_changes)} changes with conflicts that need resolution[/yellow]")
        ...
        logger.exception("Unexpected error during configuration application", error=str(e), project_path=str(project_path))
        if not structured_output:
            console.print(f"[red]Error:[/red] Unexpected error: {e}")
        raise typer.Exit(1)
...
def template_list(
    ...
    try:
        ...
        logger.info("Template listing completed", 
    ...
    try:
        ...
        if not structured_output:
            ...
            if verbose:
                ...
                console.print(f"  │   ├── pyproject.toml.template")
                console.print(f"  │   ├── .gitignore.template")
                console.print(f"  │   ├── .pre-commit-config.yaml.template")
                console.print(f"  │   └── workflows/")
                console.print(f"  ├── config.yaml")
                console.print(f"  └── version.json")
